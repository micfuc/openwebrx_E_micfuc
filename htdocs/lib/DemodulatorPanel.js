...
DemodulatorPanel.prototype.render = function () {
    var available = Modes.getModes().filter(function (m) {
        return m.isAvailable();
    });
    var normalModes = available.filter(function (m) {
        return m.type === 'analog';
    });
    var digiModes = available.filter(function (m) {
        return m.type === 'digimode';
    });
    ...
    html.push($(
            '<div class="openwebrx-panel-line openwebrx-panel-flex-line">' +
            '<select class="openwebrx-secondary-demod-listbox">' +
            '<option value="none">Digital Modes:</option>' +
            digiModes.map(function (m) {
                return '<option value="' + m.modulation + '">' + m.name + '</option>';
            }).join('') +
            '</select>' +
            '<select class="openwebrx-secondary-demod-listbox" id="change_display">' +
            '<option value="none">Displays:</option>' + '<option value="1">Timeseries</option>' + '<option value="2">Spectrum Line</option>' + '<option value="3">Spectrum Fill</option>' + '<option value="4">Waterfall Toggle</option>' +
            '</select>' +
...
DemodulatorPanel.prototype.transformHashParams = function (params) {
    ...
    if (typeof (params.offset_frequency) !== 'undefined')
        ret.offset_frequency = params.offset_frequency;
    if (typeof (params.sql) !== 'undefined')
        ret.squelch_level = parseInt(params.sql);
    return ret;
};
...
DemodulatorPanel.prototype.updateSquelch = function () {
    ...
    var demod = this.getDemodulator();
    if (demod)
        demod.setSquelch(sliderValue);
};

DemodulatorPanel.prototype.setSquelchMargin = function (margin) {
    if (typeof (margin) === 'undefined' || this.squelchMargin == margin)
        return;
